from app.device.device_button_designer import deviceButtonDesigner
...
class reportDevice(deviceButtonDesigner):
    ...
    def information_device_search(self,device):
        ...
        for oid in OID_general_info:
            answers_device.append((self.request(self.device[0],self.device[1],self.device[2],self.device[3],oid)))
        ...
        for i in range(1,int(top+1),1):
            interfaces.append(self.request(self.device[0],self.device[1],self.device[2],self.device[3],'1.3.6.1.2.1.2.2.1.2.'+str(i)))
            interfaces.append(self.request(self.device[0],self.device[1],self.device[2],self.device[3],'1.3.6.1.2.1.2.2.1.7.'+str(i)))
    ...
    def request(self,comunidad,SNMPv, puerto,ip,OID):
        iterator = getCmd(
        SnmpEngine(),
